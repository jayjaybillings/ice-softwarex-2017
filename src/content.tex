...
human input is provided. However, workflows within ICE are fully interactive with regular callbacks to humans. It is simpler to discuss ``activities'' than it is to create a
...
to perform a service lookup to identify the correct service to process the workflow.

Workflows in ICE are encoded in and processed by services called ``Items,'' and each
workflow type is a subclass of an Item base class. Items are registered dynamically
...
an ``Action Factory'' and obtained at run time. Table 1 describes the
...
API language closer to other systems. However, this must be done carefully to
...
framework specification \cite{mcaffer_osgi_2010}. ICE uses more than 1,200 additional
...
use only the workflow Items provided by their version of ICE. This extension method can seem like an onerous task to the novice user, especially if they do not know Java, but
...
ICE provides built-in development tools to automatically generate empty but ready to compile classes, (``stubs''), of
plugins that can then be installed into the framework as services. This
...
rich API that that exposes all workflow management functionality, documentation, tutorials, and tools to further simplify the process.
...
process would combine binder jetting with solid-state sintering and accounts
...
distributed via Excel macros. The new utility, developed in ICE is shown.
